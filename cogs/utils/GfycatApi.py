...
class GfycatApi(object):
    ...
    def __init__(self):
        super(GfycatApi, self).__init__()

    async def __fetch(self, url, param):
        try:
            conn = aiohttp.TCPConnector()
            headers = {"User-Agent": "Robyul"}
            session = aiohttp.ClientSession(connector=conn)
            async with session.get(url+param, headers=headers) as r:
...
if __name__ == "__main__":
    ...
    # gfycat = GfycatApi().more(param).download(save_dir)
    gfycat = GfycatApi().more(false_param).download(save_dir)
